...
{
  // float E = 1.65 * pow(10, -21); // J, za Argon msm da ne trebba ovo (vidi klasicni_sustav.c)
  float E = 1;
  // float E = 0.001;
...
{
  float product;
  // float E = 1.65 * pow(10, -21); // J, za Argon...nebitno
...
int main(void)
{
  ...
  int a, b;
  long idum = -1234;
  // BRIJEM DA MI OVE TRI NE TREBAJU NEGO NA PRAVO GI RAČUNAŠ...
  // float E0 = 20.;              // ukupna početna energija - KOLIKA?
  // float pressure = 100663.5;           // pascala... (SI unit) pressure - KOLIKI?
  // float pressure = 100000.5; // pascala... (SI unit) pressure - KOLIKI?
  // float T0 = 300;            // kelvina, temperature - KOLIKA?
  // float k_B = 1.380649 * pow(10, -23); // boltzmannova konstanta = 1.380649 × 10-23 m2 kg s-2 K-1
  // float m = 6.69 * pow(10, -26); // kg, za Argon
  // float rho = 1.784; // gustoća argona Density (at STP): 1.784 g/L == kg/metarkubni
  ...
  // float L0 = 0.1;                                   // metar, početna duljina brida spremnika L  - spremnik početni jedna litra,..
  ...
  // float x0, y0, z0, v0, vmin = 0.1, vmax = 1;
  ...
  for (i = 1; i <= Nw; i++) // po šetačima
  {
    ...
    U[i] = Upot(x, y, z, i) + m_E * Uk[i]; // valja ovo pomnožit s E pri korištenju, inače jest preračunato
  ...
  //     printf("k_b=%f; T=%f; deltaH=%f => deltaW = %f; ", k_B, T[j], delta_H, delta_W);
